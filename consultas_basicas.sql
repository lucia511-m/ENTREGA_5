...
select*from sales;--ESTA CONSULTA PERMITE VER LA TABLA COMPLETA 
--CONSULTA 2:Selección de columnas específicas para finanzas
SELECT customer_id, product_id,total_amount FROM sales; -- con esta consulta selecciono solo las que indican para visualizar 
--CONSULTA 3: Selección con alias en español para stakeholders
SELECT order_date AS fecha_pedido,SELECT product_name AS nombre_producto,SELECT quantity AS cantidad_unidades
FROM sales; -- con esta consulta selecciono columnas y las renombro de una manera mas amigable.
